...
class JSONSchemaProps(Model):
    ...
    additionalItems = Field('self', alt_type=bool)
    additionalProperties = Field('self', alt_type=bool)
    allOf = ListField('self')
    anyOf = ListField('self')
    default = Field('any')
    ...
    items = Field('self', alt_type=list)
    maxItems = Field(int)
    maxLength = Field(int)
    ...
    not_ = Field('self', name='not')
    nullable = Field(bool)
    oneOf = ListField('self')
